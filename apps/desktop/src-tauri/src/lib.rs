...
fn default_resource_dir() -> Result<PathBuf, String> {
	...
	if !candidate.is_dir() {
		return Err(format!(
			"Resource template not found at {} — run `bun install` in the repo root first.",
			candidate.display()
		));
	}
	Ok(candidate)
...
fn load_resource_meta(resource_dir: &Path) -> Result<ResourceMetaInfo, String> {
	...
	let name = pkg["name"].as_str().unwrap_or("loadscreen-resource").to_string();
	...
	let author = pkg["author"]["name"]
		...
		.unwrap_or("")
		.to_string();
	Ok(ResourceMetaInfo { name, author, version })
}
...
#[tauri::command]
fn save_project_dialog(app: AppHandle, default_name: Option<String>) -> Result<Option<String>, String> {
...
fn export_resource(app: AppHandle, files: GeneratedFiles) -> Result<ExportResult, String> {
	...
	let meta = load_resource_meta(&resource_dir)?;
	...
	create_resource_zip(&dist, &zip_path, &resource_name)?;
...
/// Zip every file under `dist` into `target`, nesting it under `resource_name/`
/// so unzipping straight into `resources/` yields `resources/<name>/…`.
fn create_resource_zip(dist: &Path, target: &Path, resource_name: &str) -> Result<(), String> {
	...
	while let Some(dir) = stack.pop() {
		...
		for entry in entries.flatten() {
			let path = entry.path();
			let rel = path.strip_prefix(dist).map_err(|_| "bad path".to_string())?;
			let name = format!("{}/{}", resource_name, rel.to_string_lossy().replace('\\', "/"));
			if path.is_dir() {
				writer
...
mod tests {
    ...
    #[test]
    fn zips_dist_with_resource_prefix() {
        let dist = Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("../..")
            .join("fivem-resource")
            .join("dist");
        let target = std::env::temp_dir().join("loadscreen-export-test.zip");
        create_resource_zip(&dist, &target, "nightfall-rp").expect("zip should build");
        ...
        assert!(
            names.iter().any(|n| n == "nightfall-rp" || n.starts_with("nightfall-rp/")),
            "entries nested under resource name, got {names:?}"
        );
        assert!(
            names.iter().any(|n| n.ends_with("server.js")),
            "compiled server.js included, got {names:?}"
        );
        assert!(
            names.iter().any(|n| n.ends_with("load.html")),
            "loading screen html included, got {names:?}"
        );
